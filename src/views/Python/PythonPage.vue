<template>
  ...
  <div>
    <div v-if="$route.path === '/python'" class="introduce">
      <appSider>
        ...
        <div>
          ...
          的通用编程语言。
          <popUp :data="popupData[0]" :ref="(el:any) => (popupRefs[0] = el)"> </popUp>
          在今天我们使用python的理由很多,比如
          <el-button type="info" @click="showPopup(1)">快速原型</el-button>
          <popUp :data="popupData[1]" :ref="(el:any) => (popupRefs[1] = el)"></popUp>
        ...
        <ul>
          <li v-for="index in [2, 3, 4, 5]" :key="index">
            ...
            <popUp :data="popupData[index]" :ref="(el:any) => (popupRefs[index] = el)" />
          </li>
        </ul>
        <div id="container">
    ...
    <div v-else>
      <appSider><router-view></router-view></appSider>
    </div>
  </div>
</template>
...
print(nums)`,
  ...
  buildViewBlocks(
    '算法开发',
    [
      '这里不得不提到一个名词——心智开销。由于 Python 本身的语法自由度极高，即便 Python 在某些算法场景下存在速度上的性能压力，多数时候人们还是会选择 Python 作为算法开发的首选语言。比如，在学术研究中，很少有人会直接使用 C++ 来写算法，因为 C++ 在书写时需要额外管理内存之类的附属操作——而内存管理这类问题，在学术算法原型阶段并不是关注的重点。但在工业界，C++ 的算法开发确实是金字招牌。',
